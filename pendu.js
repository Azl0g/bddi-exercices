...
class Game {
  constructor() {
    ...
    function init() {
      ...
      word = WORD_LIST[Math.floor(Maths.random() * WORD_LIST.length)]; // TODO: récupérer un mot random depuis WORD_LIST (1 ligne)
      console.log(word);
        letters = []
    ...
    function addLetter(letter) {
     ...
     // if(word.includes(letter)){
      ...
      letters.push(letter);
      if(word.includes(letter)) missing--;
      else lives--;
    ...
    function displayWord() {
      ...
      for (let i = 0, len = word.length; i < len; i++) {
        let letter = word[i]
        wordToDisplay += letters.includes(letter) ? letter : '-'
      }
    }
